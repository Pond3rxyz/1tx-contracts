...
/// @title MorphoAdapter
/// @notice Adapter for managing Morpho Vaults V2 (ERC-4626 compliant)
/// @dev Implements the ILendingAdapter interface for Morpho Vaults V2
contract MorphoAdapter is AdapterBase {
    ...
    /// @notice Configuration for a single Morpho vault
    struct VaultConfig {
        Currency currency;
    ...
    /// @notice Emitted when a deposit is made to Morpho Vault
    event DepositedToMorpho(bytes32 indexed marketId, uint256 assets, uint256 shares, address onBehalfOf);

    /// @notice Emitted when a withdrawal is made from Morpho Vault
    event WithdrawnFromMorpho(bytes32 indexed marketId, uint256 assets, uint256 shares, address to);
    ...
    /// @notice Registers a new Morpho Vault in this adapter
    ...
    function registerVault(Currency currency, address vault) external onlyOwner validCurrency(currency) {
        ...
        // This enables different strategies/vaults for the same asset (e.g., multiple USDC vaults)
    ...
    function getAdapterMetadata() external view override returns (AdapterMetadata memory metadata) {
        return AdapterMetadata({name: "Morpho Vaults V2", chainId: block.chainid});
    }
    ...
    /// @notice Deposits tokens into Morpho Vault (ERC-4626)
    /// @param marketId The market identifier
        ...
        validDepositWithdrawParams(amount, onBehalfOf)
    {
        ...
        emit DepositedToMorpho(marketId, amount, shares, onBehalfOf);
    }

    /// @notice Withdraws tokens from Morpho Vault (ERC-4626)
    /// @param marketId The market identifier
        ...
        returns (uint256 assetsWithdrawn)
    {
        ...
        emit WithdrawnFromMorpho(marketId, assetsReceived, amount, to);
