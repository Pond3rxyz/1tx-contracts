...
contract SwapDepositRouterForkTest is AdapterForkTestBase {
    ...
    function test_fork_buy_withSwap_usdbc() public {
        ...
        vm.prank(user);
        uint256 deposited = router.buy(usdbcInstrumentId, DEPOSIT_AMOUNT, false, 0);
    ...
    function test_fork_sell_noSwap_usdc() public {
        ...
        vm.prank(user);
        uint256 output = router.sell(usdcInstrumentId, aTokenBalance);
    ...
    function test_fork_sell_withSwap_usdbc() public {
        ...
        vm.prank(user);
        router.buy(usdbcInstrumentId, DEPOSIT_AMOUNT, false, 0);
        ...
        uint256 usdcBefore = _getBalance(usdc, user);
        vm.prank(user);
        uint256 output = router.sell(usdbcInstrumentId, aTokenBalance);
